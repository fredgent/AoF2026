...
\usepackage{natbib} 
% mm   \bibliographystyle{aasjournal} [this doesn't play well with
% plain article style, producing the lack of space between author and
% year]
...
We plan to implement groundbreaking 
% mm coupled multi-tiered, or 
{\textit{extended
multi-scale simulations}} (EMS), bridging previously unattainable scales of
separation. Extreme scales of separation are generic in astrophysics, where
% mm 
   we shall apply 
this innovative concept 
% mm shall be applied 
to dust, but our new interactive
control tools 
% mm would have applicability in other disciplines proliferated by
  will apply to other disciplines characterized by
large and heterogeneous data.
...
% mm densities
  pressure gradients
% mm change \cite to \citep or \citet to avoid odd formatting in the PDF
exceed some threshold \citep{AMR00}.  However, in MHD the
...
turbulent fields are challenging to determine \citep{NUFFT}. Multi-grid methods
are also commonly employed for improving convergence with implicit solvers of

Another resource applying a common interface to multiple astrophysics
% mm
  algorithms
is the Astrophysical Multipurpose Software Environment \citep{AMUSE}.  Its Python
...
\begin{minipage}[h]{0.29\textwidth}
  \caption{\footnotesize \label{ISMsim}\label{ISMsim2} Typical snapshots of gas
number $n$ and magnetic energy $e_B$ density and magnetic field $B_y$ from an
MHD simulation of SN-driven turbulence and galactic dynamo \citep{GMK24}.
Turbulence drives a fast small-scale dynamo, which can create magnetic fields
within a few percent of the kinetic energy of the turbulence, while
stratification about the galactic disc regulates the multi-phase structure of
the ISM via outflows and inward precipitation. Large-scale shear is required
for a mean-field dynamo, which continues the growth of the magnetic field to
energy equipartition with its kinetic energy.  }
\end{minipage}
\end{figure}
%==============================================================================
...
by some as a {\textbf{dust budget crisis}} \citep{Rowlands14b}.  Sublimation,
condensation, grain-grain interactions, ion sputtering are well established as
...
omit realistic ISM turbulence.  The treatment of dust abundances significantly
affects the galaxy evolution and the properties of the circum-galactic medium
in cosmological scale simulations arizing from cooling \citep{Babul23}.

Simulations to sixth-order accuracy explore many scales of the ISM.  Due to a
vast range of scale in space and time all models, full galaxy or inside
star-forming molecular clouds (MC), must exclude some physics from their scope.
Models at kpc-scale (see, e.g., Figure~\ref{ISMsim}) of SN-driven turbulence on
stratified discs with galactic-shear dynamo \citep[e.g.,][]{ GMK24} omit dust
processing, but acquire the necessary turbulent structure.  Dust would decouple
from the gas \citep[][]{Mattsson18a} and, because grains are easily charged,
radiation pressure and Lorentz forces affect its critical MC scale
distribution.
...
average density for each grain size in each specimen.  At region ({\textbf{c}})
significant amount of dust grains are removed or destroyed by the shock while
at regions ({\textbf{b}}) and ({\textbf{d}}) any change is due to background
processing (transport and destruction).  }
  \label{fig_gsd}
%\end{figure}
\end{minipage}

\fag{Add here summary of research questions/hypotheses}

The primary research questions of the project are to quantify the rates at
which dust is processed by Supernova blast waves - how this depends on the
multiphase turbulence of the ISM and on the strength and topology of the
galactic magnetic field. By including realistic large scale fields and
large scale galactic topology in increasingly high resolution zoom-in of the
ISM we aim also to quantify where and when the blast waves are most destructive.We shall clarify which processes in the ISM are most critical to dust
losses and properties and regions of the ISM which most mitigate such
losses.

The suitability of this project for international collaboration in HPC has been
demonstrated by successful campaigns at the LUMI Extreme Sscale Access 2025
call \citep{GMKPR26} and the INCITE 2026 award on Frontier (USA), which is
currently progressing. This project will extend those simulations to include
dust processing, which can require much higher time resolution and large scale
dynamos which require larger domains and much longer integration duration.
Acceleration with GPUs on Lumi is critical to the feasibility of such studies.
The PC-A code has obtained at over 20 times speed-up over the equivalent PC
CPU-based code. It has performed reliably and stably with weak scaling
approaching a constant execution time up to 4096 GPU cores on Frontier.

It is a primary focus of this project to bring together two or more codes, or
instances of the same code, working on related but distinct physics. The
participants will develop internationally leading expertise in knowledge and
application of our collaborative codes and how to create, adapt and apply
machinery to support their synthesis.
...
\fag{Check that risk assessment and alternative strategies are included}
...
such as self-gravity or stellar radiation and, eventually, dust.
 Applying the same such machinery with
respect to other codes, we aim in future to embed our largest scale dynamo models in
galaxy scale codes such as \textsc{Arepo} or \textsc{Romulus} used by our Victoria
collaborators \citep{Babul21L,Babul21a}, to capture even larger scale structure
of the ISM magnetic field.
  EMS is not
...
$\mathcal{D}_0$, enclosing region $\mathcal{R}_n$ with $n=1$.\\%[-0.7cm]
    \item Identify region $\mathcal{R}_n$ (e.g.,
% mm dense star-forming cluster
     star forming cloud), 
small enough to ensure scale separation.\\%[-0.7cm]
...
\tabto{0em}
$\bullet$~Hand setting $\mathcal{D}_n$ \& boundary conditions superseded by pipeline tools (see \wpref{3}).
\tabto{0em}
$\bullet$~Post-processed dust (\textsc{PAPERBOATS}) superseded by integral \textsc{Pencil Code}
 (\wpref{2}) and graphics cards (GPU)-accelerated sub-physics and dust (\wpref{4}).
%------------------------------------------------------------------------------
...
codes or computations require resource management to avoid idle resources, match performance and reduce IO, memory or compute bottlenecks (\wpref{3}).
\end{minipage}

To implement \wpref{1} and \wpref{2} the PI and CoI have years of experience executing
...
to PC-A are low risk, as the PI and CoI lead the HPCLab development of the
PC-A implementation of SN driven turbulence, and partial dust-processing
modeling has successfully been implemented by the Swedish collaborator.
...
the better the success of \wpref{3} and \wpref{4}.  The GS shall be included in
the execution and analysis of \wpref{1} and \wpref{2}, while their thesis shall
primarily address \wpref{3} and \wpref{4}.
Our collaborators from Victoria, New York, Stockholm and Ghent
...
%\vspace{-0.3cm}
%Scanning for regions of interest in
%astrophysical simulations remains labour intensive and applied to disk-saved
%datasets, output at intervals.
%We shall apply cutting-edge machine-learning
...
%------------------------------------------------------------------------------
%------------------------------------------------------------------------------
\wpdef{3}{CC}{Pipeline -- code coupling}
\vspace{-0.3cm}
...
$\mathcal{D}_{n}$ BC updates.

Data storage and management is commonly a bottleneck in any HPC, dealing with
datasets approaching TB and even PB scales.
 Code-coupling on-the-fly, saves on storage, and
...
communication.
We have access to cutting-edge machine-learning
tools for Opportunistic Data Operations \citep[][ODOP]{odop24} to analyze the
data to identify regions or patterns of interest and extract a suitable
subdomain from which to launch new experiments.
This work is at the frontier of computer science, in which HPCLab
...
The basic machinery for coupling multiple versions of the code
...
\begin{figure}[h]
    \begin{ganttchart}{1}{18}
      ...
      \gantttitle{Year 3}{6} \\
%-----------------------------------------------------------------------------
      \ganttgroup{\wpref{1} (EMS)}{1}{18} \\
      \ganttbar{$R_1\rightarrow R_n$}{1}{4}\\
      \ganttbar{Coupled $R_1\rightarrow R_n$}{6}{18}\\
...
%-----------------------------------------------------------------------------
      \ganttgroup{\wpref{2} (Dust)}{1}{18} \\
      \ganttbar{\textsc{Paperboats dust}}{1}{4} \\
      \ganttbar{\textsc{PC dust}}{6}{9}\\
      \ganttbar{PC-A dust}{10}{18}\\
      \ganttmilestone[milestone/.append style={fill=blue}]{\textsc{Dust in PC/PC-A}}{4}
      \ganttmilestone[milestone/.append style={fill=green}]{\textsc{}}{9} \\
%%-----------------------------------------------------------------------------
%      \ganttgroup[group/.append style={fill=yellow}]{\wpref{3} (ODOP)}{1}{4}
%      \ganttgroup[group/.append style={fill=yellow}]{}{12}{17} \\
%      \ganttbar[bar/.append style={fill=yellow}]{dev. ODOP selection}{1}{4} \\
%      \ganttbar{ODOP applied}{6}{8} \\
%      \ganttbar[bar/.append style={fill=yellow}]{ODOP coupling}{12}{14}
%      \ganttbar{}{15}{17} \\
%      \ganttmilestone[milestone/.append style={fill=yellow!50}]{publication}{9}
%      \ganttmilestone[milestone/.append style={fill=yellow!50}]{publication}{16} \\
%-----------------------------------------------------------------------------
      \ganttgroup[group/.append style={fill=yellow}]{\wpref{3} (Coupling)}{1}{4}
      \ganttgroup[group/.append style={fill=yellow}]{}{10}{18} \\
      \ganttbar[bar/.append style={fill=yellow}]{Dev. coupling PC}{1}{4} \\
      \ganttbar[bar/.append style={fill=yellow}]{GPU/other coupling}{10}{18} \\
%      \ganttbar{coupling applied}{5}{7}
%      \ganttbar{}{16}{18} \\
%-----------------------------------------------------------------------------
      \ganttgroup[group/.append style={fill=green}]{\wpref{4} (GPU)}{5}{9}\\
      \ganttbar[bar/.append style={fill=green}]{Dev. PC-A dust}{5}{9} \\
      \ganttmilestone[milestone/.append style={fill=blue!50}]{Publications}{4}
      \ganttmilestone[milestone/.append style={fill=blue!50}]{}{9}
      \ganttmilestone[milestone/.append style={fill=blue!50}]{}{18}
      \ganttmilestone[milestone/.append style={fill=yellow!50}]{}{5}
      \ganttmilestone[milestone/.append style={fill=yellow!50}]{}{15}
      \ganttmilestone[milestone/.append style={fill=blue!50}]{}{13}
      \ganttvrule{}{6}
      \ganttvrule{}{12}
    \end{ganttchart}
    \vspace{-1.6em}
    \caption{
Gantt chart: proposed 3-year timeline and \wpref{} organization. Blue denotes
science applications, results executed with PC and PC-A; yellow denotes
development and application of coupling and EMS management tools; green denotes GPU
integration into PC. At least 3 publications shall
...
The GS with the support of the PI and CoI will implement code coupling (\wpref{4})
...
in our recent extreme-scale simulations applied on LUMI \citep{GMKPR26} and on
Frontier \href{https://www.olcf.ornl.gov/2025/04/11/incite-2026-call-for-proposals/}{2026 INCITE call}.  This utilizes the higher functionality of
...
%------------------------------------------------------------------------------
%------------------------------------------------------------------------------
\subsection{Research data and material, methods and research environment}

The research data is mainly simulated in HDF5 or ASCII form of TB scale, although some
astrophysical observational data may be analyzed alongside our generated data for
scientific publication. We also produce and utilize open source numerical codes
and additional open source software tools for code integrations, management
pipelines and data analysis. The primary code milestones of this project will include: the implementation of
Paperboats advanced dust processing physics into the Pencil Code and Astaroth for accelerated
parallel 3D integration; code coupling machinery for running multiple versions of PC/PC-A or  to match
difficult combinations of scales and physics; automation to support
code coupling and simulation management.

The main research environment is the Department of Computer Science (hereafter Aalto/CS) at the
Aalto University School of Science. With over 40 professors and more than 400 employees, the
department is the largest Computer Science unit in Finland. It is ranked among the top 10 Computer
Science departments in Europe and among the top 100 globally. ICT and digitalization are among
the seven key research areas at Aalto University. In addition to standard resources provided by the
department, there is support for midrange HPC and data analytics through the Science-IT project of
the Aalto University School of Science. Science-IT and its ﬂagship HPC unit Triton, are a part of the
national Finnish Computing Competence Infrastructure (FCCI). Currently, Triton provides 9600
computing cores and over 30 servers for high-end GPU computing. For indepth research methods
and software development, Science-IT provides support from research software engineers.
Researchers also have free access to national CSC resources, including the supercomputers Puhti
and Mahti, as well as the cloud resources Rahti and CPouta. The teams also have access to Europe’s
most powerful GPU-based supercomputer LUMI through competitive computing time allocations
via the Euro-HPC consortium. The consortium has an outstanding track record of obtaining
competitive computational resources through the Partnership for Advanced Computing in Europe
(PRACE), EuroHPC, and CSC.

The research infrastructure extends to mature longterm collaborations between the
PI, CoI and AaltoHPC on MHD and galaxy modelling with New York and Victoria collaborators
and the CoI on dust processing with the Stockholm and Ghent collaborators.
...
\begin{tabular}{|m{4.0cm}|m{5.3cm}|m{7.0cm}|}
\hline
Collaborator & Institute & Role \\
\hline
\href{https://nordita.org/people/nordita-person/lars-mattsson}{{Lars Mattsson}}\href{https://orcid.org/0000-0003-2670-2513}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} & Nordita \& Stockholm University & Modeling dust processing dynamics \\[0.1cm]
\href{https://kirchschlager.github.io/}{{Florian Kirchschlager}}\href{https://orcid.org/0000-0002-3036-0184}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} & Ghent University & Modeling dust processing dynamics \\[0.1cm]
\href{https://www.uvic.ca/research/centres/arc/people/faculty/babul-arif.php}{{Arif Babul}}\href{https://orcid.org/0000-0003-1746-9529}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} & University of Victoria & Galaxy evolution and composition \\[0.1cm]
%\href{https://science.gsfc.nasa.gov/astrophysics/cosmology/bio/eliahu.dwek-1}{{Eli Dwek??}}\href{https://orcid.org/0000-0001-8033-1181}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} & NASA Emeritus Professor & Supernova Remnants \& ISM and Dust \\[0.1cm]
\href{https://www.amnh.org/research/staff-directory/mordecai-mark-mac-low}{Mordecai-M Mac Low}\href{https://orcid.org/0000-0003-0064-4060}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} & American Museum of Natural History, New York & Modeling astrophysical turbulence and physics of molecular clouds \\[0.4cm]
\href{https://www.aalto.fi/en/people/linh-truong?check_logged_in=1}{Hong-Linh Truong}\href{https://orcid.org/0000-0003-1465-9722}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} & Aalto University & analytics of distributed software systems, big data applications \& machine learning tools (ODOP)\\[0.4cm]
\hline
\end{tabular}

The Principal Investigator (PI) Maarit Korpi-Lagg\href{https://orcid.org/0000-0002-9614-2200}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}}
(over 4,800 citations and h-index of 39 according to Google Scholar), is a
tenured associate professor at Aalto CS, where she leads the High-Performance Computing Lab. She
was the grant holder of the ERC Consolidator grant "UniSDyn" and the ERC Proof of-concept grant
“SYCOS”, which projects studied dynamo action in solar and stellar convection zones. She acted as a
team leader of the Academy of Finland Centre of Excellence “ReSoLVE” (2014-2019), which studied
long-term solar magnetic activity and its eﬀects on Earth. She has acted as a PI of several
competitive HPC projects (LUMI-C and LUMI-G Finnish project allocations, PRACE, DEISA/DECI, CSC
Grand Challenge), and is a core developer of the Pencil Code Multiphysics Solver [8] for high-order
stencil-based applications, and the initiator and project leader of the GPU-based MHD solver
Astaroth.

The Co-Investigator
(CoI) Frederick Gent\href{https://orcid.org/0000-0002-1331-2260}{\includegraphics[scale=0.5]{orcid_16x16.jpeg}} has been
...
There are no ethical concerns as the application deals with no sensitive data.
Nevertheless, we will follow stringent integrity principles in accordance with
the guidelines of the Finnish Advisory Board on Research Integrity, and the
ALLEA European Code of Conduct for Research Integrity.  We are committed to
promote diversity, equality and non-discrimination in all our activities.
Statutory obligations set forth in the Act on Equality between Women and Men
609/1986, 1329/2014 and the Non-discrimination Act 1325/2014 also apply to our
plan. We are ﬁrmly committed to promote equal opportunities for individuals to
learn, acquire knowledge, participate and make a diﬀerence. We will make our
values visible in all our practices.

The products of the research will be made available immediately to the
community during the project. Fully open access publications, data, and source
code policy will be adopted. Services like Zenodo and FAIRdata IDA will be
utilised for opening up data products ranging from the data to reproduce all
the ﬁgures, to intermediate and selected raw data products for reproducibility
and reusability of the results. All the simulation and data analysis tools are
developed as open source, available and continuously maintained under GitHub
and Bitbucket repositories.

Our simulation and data analysis tools are being developed to work generally on
GPUs and hence are energy-eﬃcient in comparison to standard CPU computing.
Data-eﬃcient methods in our work help to achieve sustainable storage (also
avoid data waste) and energy eﬃciency. Furthermore, we assess if international
and national travel can be avoided by using remote visits. Computational time
will be used wisely to avoid too large energy consumptions.
%------------------------------------------
%%%% REFERENCES %%%%

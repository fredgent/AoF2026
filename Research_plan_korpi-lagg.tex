...
unattainable scales of separation. Extreme scales of separation are
generic in astrophysics, where we shall apply this innovative concept
to dust
% mm
  formation and dynamics,
but our new interactive control tools
% mm would have applicability in
  will apply to
other disciplines
% mm proliferated
  characterized
by large and heterogeneous data.
...
highly non-linear systems in chemistry \citep{MGRID}, engineering and other
...
%\small
\begin{enumerate}
\item\tabto{0em} Select
  % mm or extend
  a suitable macro simulation on domain
...
\item Identify region $\mathcal{R}_n$ (e.g.,
  ...
  star-forming cloud), small enough to ensure scale separation.%\\[-0.7cm]
...
to the desired resolution.% as initial conditions (ICs).\\[-0.7cm]
    \item Define appropriate boundary conditions. Interpolate from
$\mathcal{D}_0$ to retain macro dynamics.%\\[-0.7cm]
    \item Run simulation of $\mathcal{D}_n$.%\\[-0.7cm]
    \item Repeat steps 2--5 for $n>1$ until the size of $\mathcal{D}_n$
